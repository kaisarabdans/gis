...
$db = new ezSQL_mysqli($config['username']='root', $config['password']='', $config['database_name']='gis', $config['server']='localhost');
...
function hapus_transportasi($ID){
    global $db;
    $row = $db->get_row("SELECT gambar FROM tb_transportasi WHERE id_kendaraan='$ID'");
    if($row){
        $file1 = 'assets/images/galeri/' . $row->gambar;
        $file2 = 'assets/images/galeri/small_' . $row->gambar;
        if(is_file($file1))
